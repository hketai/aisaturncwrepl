  ...
  <div class="flex flex-col w-full gap-2 max-h-[312px]">
    ...
    <div class="grid grid-cols-2 gap-x-3 gap-y-2 w-full auto-rows-[47px]">
      <button
        v-for="year in years"
        :key="year"
        class="p-2 text-sm font-medium text-center text-slate-900 w-[144px] h-10 rounded-lg py-2.5 px-2"
        :class="{
          'bg-indigo-600 text-white hover:bg-indigo-900': year === activeYear,
          'hover:bg-slate-900/10 dark:hover:bg-slate-300': year !== activeYear,
        }"
