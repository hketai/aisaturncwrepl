...
<template>
  <form @submit.prevent="handleSubmit">
    <div class="flex flex-col gap-4 mb-2 divide-y divide-slate-300">
      <BaseInfo
    ...
    <div
      ...
      class="flex flex-col gap-4 divide-y divide-slate-300 border-t border-slate-300 mt-6"
    >
      <InboxCapacityLimits
      ...
      <div v-if="showUserSection" class="py-4 flex-col flex gap-4">
        <div class="flex items-end gap-4 w-full justify-between">
          <div class="flex flex-col items-start gap-1 py-1">
            <label class="text-sm font-medium text-slate-900 py-1">
              {{ t(`${BASE_KEY}.FORM.USERS.LABEL`) }}
            </label>
            <p class="mb-0 text-slate-900 text-sm">
