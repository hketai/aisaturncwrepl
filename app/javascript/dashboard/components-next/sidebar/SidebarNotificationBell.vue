...
<template>
  <button
    class="size-8 rounded-lg hover:bg-slate-900/5 flex-shrink-0 grid place-content-center relative"
    @click="openNotificationPanel"
  >
    <span class="i-lucide-bell size-4" />
    <span
      v-if="unreadCount"
      class="min-h-2 min-w-2 p-0.5 px-1 bg-red-800 rounded-lg absolute -top-1 -right-1.5 grid place-items-center text-[9px] leading-none text-red-200"
    >
      {{ unreadCount }}
    </span>
  </button>
