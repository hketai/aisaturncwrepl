...
<template>
  <BaseBubble class="px-4 py-3" data-bubble-name="csat">
    ...
    <dl v-if="isRatingSubmitted" class="mt-4">
      <dt class="text-slate-900 italic">
      ...
      <dd v-else class="flex mt-1">
        <span v-for="n in 5" :key="n" class="text-2xl mr-1">
          <i
            :class="[
              n <= starRatingValue
                ? 'i-ri-star-fill text-amber-800'
                : 'i-ri-star-line text-slate-900',
            ]"
          />
        </span>
      </dd>

      <dt v-if="response.feedbackMessage" class="text-slate-900 italic mt-2">
