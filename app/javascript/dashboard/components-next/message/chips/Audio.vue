...
<template>
  ...
  <div
    v-bind="$attrs"
    class="rounded-xl w-full gap-2 p-1.5 bg-white/70 flex flex-col items-center border border-slate-300 shadow-[0px_2px_8px_0px_rgba(94,94,94,0.06)]"
  >
    <div class="flex gap-1 w-full flex-1 items-center justify-start">
      ...
      <div class="flex-1 items-center flex px-2">
        <input
          ...
          :max="duration"
          :value="currentTime"
          class="w-full h-1 bg-slate-900/40 rounded-lg appearance-none cursor-pointer accent-current"
          @input="seek"
        />
      </div>
      <button
        class="border-0 w-10 h-6 grid place-content-center bg-slate-900/10 hover:bg-alpha-3 rounded-2xl"
        @click="changePlaybackSpeed"
      >
        <span class="text-xs text-slate-900 font-medium">
          {{ playbackSpeedLabel }}
        </span>
      </button>
      <button
    ...
    <div
      ...
      class="text-slate-900 p-3 text-sm bg-slate-900/5 rounded-lg w-full break-words"
    >
