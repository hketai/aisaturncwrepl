...
<template>
  ...
  >
    ...
    <div
      v-if="showPopover"
      ref="dropdownRef"
      class="z-50 flex flex-col items-start absolute bg-slate-900/15 backdrop-blur-[50px] border-0 gap-4 outline outline-1 outline-slate-300 rounded-xl max-w-96 min-w-80 max-h-[20rem] overflow-y-auto py-2"
      :class="[
        shouldShowAbove ? 'bottom-full mb-2' : 'top-full mt-2',
        shouldAlignRight ? 'right-0' : 'left-0',
      ]"
    >
      <div class="flex flex-col divide-y divide-slate-300 w-full">
        <Input
          v-model="searchValue"
        ...
        >
          <template #prefix>
            <Icon
              ...
              class="absolute -translate-y-1/2 text-slate-900 size-4 top-1/2 ltr:left-3 rtl:right-3"
            />
          </template>
        </Input>
        ...
        <div
          ...
          :key="item.id"
          class="flex gap-3 min-w-0 w-full py-4 px-3 hover:bg-slate-900/10 cursor-pointer"
          ...
          <div class="flex flex-col items-start gap-2 min-w-0 flex-1">
            <div class="flex items-center gap-1 min-w-0 w-full">
              <span
                ...
                class="text-sm text-slate-900 truncate min-w-0 flex-1"
              >
            ...
            <span
              ...
              class="text-sm text-slate-900 truncate min-w-0 w-full block"
            >
