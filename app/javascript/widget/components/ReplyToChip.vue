  ...
  methods: {
    navigateTo(id) {
      ...
      this.$nextTick(() => {
        ...
        el.scrollIntoView();
        el.classList.add('bg-slate-200', 'dark:bg-slate-300');
        this.timeOutID = setTimeout(() => {
          el.classList.remove('bg-slate-200', 'dark:bg-slate-300');
        }, 500);
      });
    },
  },
...
<template>
  <button
    class="px-1.5 py-0.5 rounded-md text-slate-900 bg-slate-300 opacity-60 hover:opacity-100 cursor-pointer flex items-center gap-1.5"
